...
const allNavItems = uniqueNavItems([...inriNavItems, ...utilityNavItems])
...
function NavLink({ item }: { item: InriNavItem }) {
  ...
  return (
    <Link
      href={item.href}
      className={`inline-flex h-11 items-center justify-center rounded-[14px] px-4 text-[14px] font-extrabold transition ${
        active
          ? 'border border-sky-200 bg-sky-50 text-sky-700 shadow-[0_12px_24px_rgba(14,165,233,0.08)]'
  ...
  return (
    <Sheet>
      <SheetTrigger asChild>
        <button className="inline-flex h-11 w-11 items-center justify-center rounded-[14px] border border-slate-200 bg-white/90 text-slate-700 shadow-[0_14px_26px_rgba(15,23,42,0.08)] transition hover:border-sky-300 hover:text-sky-700 lg:hidden">
          <Menu className="h-5 w-5" />
          <span className="sr-only">Open menu</span>
        </button>
      </SheetTrigger>
      <SheetContent side="right" className="w-[88vw] max-w-md border-l border-slate-200 bg-white p-0 text-slate-900">
        <SheetHeader className="border-b border-slate-200 px-5 py-5 text-left">
          ...
          <SheetDescription className="pt-2 text-left text-slate-500">Official routes for the INRI ecosystem.</SheetDescription>
        </SheetHeader>
        <div className="space-y-6 px-5 py-5">
          <div className="grid gap-3">
            <InriLinkButton href={LIVE_WALLET_URL} external noTranslate>INRI Wallet</InriLinkButton>
            <InriLinkButton href={EXPLORER_URL} external variant="secondary" noTranslate>Explorer</InriLinkButton>
            <div className="w-full min-w-0"><ConnectWalletButton compact /></div>
          </div>
...
export function InriHeader() {
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200/90 bg-white/88 shadow-[0_10px_40px_rgba(15,23,42,0.06)] backdrop-blur-xl">
      <div className="border-b border-sky-100 bg-[linear-gradient(90deg,rgba(125,211,252,0.45),rgba(255,255,255,0.95),rgba(125,211,252,0.45))]">
        <div className="mx-auto flex max-w-[1600px] items-center justify-center px-4 py-2.5 sm:px-8 xl:px-12 2xl:px-16">
          <p className="text-center text-[10px] font-black uppercase tracking-[0.22em] text-sky-800 sm:text-[12px] lg:tracking-[0.28em]">
            INRI Mainnet · Proof-of-Work · Chain 3777 · EVM Compatible
          </p>
        </div>
      </div>
      <div className="mx-auto max-w-[1600px] px-4 sm:px-8 xl:px-12 2xl:px-16">
        <div className="grid min-h-[80px] grid-cols-[auto_1fr_auto] items-center gap-3 py-3 lg:gap-6 lg:py-0">
          ...
          <div className="hidden min-w-0 items-center justify-center lg:flex">
            <nav className="flex items-center gap-1 rounded-[18px] border border-slate-200 bg-white/85 p-1.5 shadow-[0_12px_28px_rgba(15,23,42,0.05)]">
          ...
          <div className="flex items-center justify-self-end gap-2 md:hidden">
            <div className="hidden min-[430px]:block">
              <InriLinkButton href={LIVE_WALLET_URL} external noTranslate variant="secondary">Wallet</InriLinkButton>
            </div>
            <MobileMenu />
          </div>
        </div>
...
function BackToTopButton() {
  ...
  return (
    <button
      ...
      className={`inri-back-to-top fixed bottom-5 right-5 z-[60] inline-flex h-12 w-12 items-center justify-center text-sky-700 transition-all hover:-translate-y-1 hover:text-sky-800 sm:bottom-7 sm:right-7 ${
...
export function InriShell({ children }: { children: ReactNode }) {
  return (
    <div className="inri-shell-bright">
      <InriHeader />
      {children}
      <InriFooter />
      <BackToTopButton />
...
function FooterSocialIcon({ link }: { link: SocialLink }) {
  return (
    <Link
      ...
      title={link.label}
      className="inline-flex h-11 w-11 items-center justify-center rounded-[14px] border border-slate-200 bg-white text-slate-600 shadow-[0_10px_24px_rgba(15,23,42,0.05)] transition-all hover:-translate-y-px hover:border-sky-300 hover:text-sky-700"
    >
      {link.icon}
    </Link>
  )
...
export function InriFooter() {
  return (
    <footer className="border-t border-slate-200 bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(241,248,255,0.98))]">
      <div className="mx-auto max-w-[1600px] px-4 py-16 sm:px-8 lg:py-20 xl:px-12 2xl:px-16">
        <div className="grid gap-12 lg:grid-cols-[minmax(0,1.2fr)_repeat(4,minmax(0,0.75fr))] lg:gap-10">
          <div className="max-w-md">
            ...
            <p className="mt-6 text-sm leading-7 text-slate-600 sm:text-[15px]">
              INRI CHAIN official surface: wallet, explorer, mining, staking, token launch, P2P and championship routes from one brighter and more consistent interface.
            </p>
            ...
            <div className="mt-8 h-px w-full bg-gradient-to-r from-sky-300 via-slate-200 to-transparent" />
          ...
          {footerGroups.map((group) => (
            <div key={group.title}>
              <h3 className="text-[1.05rem] font-black uppercase tracking-[0.14em] text-slate-900">{group.title}</h3>
              <div className="mt-6 grid gap-4">
                {group.links.map((item) => (
                  <Link
                    key={`${group.title}-${item.label}-${item.href}`}
                    href={item.href}
                    className="inri-bright-footer-link text-[15px] font-semibold"
                    {...(item.external ? { target: '_blank', rel: 'noreferrer' } : {})}
                  >
                    {item.label}
                  </Link>
          ...
          <div>
            <h3 className="text-[1.05rem] font-black uppercase tracking-[0.14em] text-slate-900">Network</h3>
            <div className="mt-6 grid gap-3 text-[15px] text-slate-600">
              <div className="border-l-2 border-sky-300 pl-3">
                <p className="text-xs uppercase tracking-[0.16em] text-slate-400">Consensus</p>
                <p className="mt-1 font-black text-slate-900">Proof-of-Work</p>
              </div>
              <div className="border-l-2 border-sky-300 pl-3">
                <p className="text-xs uppercase tracking-[0.16em] text-slate-400">Chain ID</p>
                <p className="mt-1 font-black text-slate-900">3777</p>
              </div>
              <div className="border-l-2 border-sky-300 pl-3">
                <p className="text-xs uppercase tracking-[0.16em] text-slate-400">Compatibility</p>
                <p className="mt-1 font-black text-slate-900">EVM</p>
              </div>
              <Link href="mailto:contact@inri.life" className="mt-2 font-black text-slate-900 transition hover:text-sky-700">
                contact@inri.life
              </Link>
            </div>
          </div>
